...
"""
Created on Fri Dec 09 16:26:10 2016
...
def main():
    ...
    print('Build model...')
    model = km.Sequential()
    model.add(kl.LSTM(128, input_shape=(maxlen, len(chars))))
    model.add(kl.Dense(len(chars)))
    model.add(kl.Activation('softmax'))
    optimizer = ko.RMSprop(lr=10000)
    model.compile(loss='categorical_crossentropy', optimizer=optimizer)
    
    '''train the model, output generated text after each iteration'''
    f = open('reveries.txt', 'w')
